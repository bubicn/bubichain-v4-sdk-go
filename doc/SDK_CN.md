...
## 概述
本文档详细说明BubiChain Go SDK常用接口文档, 使开发者更方便地操作和查询BU区块链。

## 包导入

go必须是1.10.1或更高版本。

项目所依赖的包在src文件夹中，获取包由 BUBI 提供。
...
这里的操作是指在交易中做的一些动作，便于序列化交易和评估费用。操作详情请见[操作](#操作)。例如，构建发送Gas操作(`GasSendOperation`)，接口调用如下：
```go 
...
```go
//初始化传入参数
PrivateKey := []string{"privbwAQyE2vWwzt9NuC8vecqpZm7DS8kfiMPsKPcrTatUkmkxkVhfaf"}
...
- **请求参数**
   ...
   gasPrice|int64|必填，交易燃料单价，单位UGas，1 Gas = 10^8 UGas，大小限制[1000, max(int64)]
   feeLimit|int64|必填，交易要求的最低的手续费，单位UGas，1 Gas = 10^8 UGas，大小限制[1, max(int64)]
...
- 成员
   ...
   initBalance   |   int64   |  必填，初始化资产，单位UGas，1 Gas = 10^8 UGas, 大小(0, max(int64)] 
   metadata|String|选填，备注
...
- 成员
   ...
   sourceAddress|String|选填，操作源账户地址
   destAddress|String|必填，目标账户地址
   gasAmount|int64|必填，资产发行数量，大小限制[0, max(int64)]
   metadata|String|选填，备注
...
- 成员
   ...
   ------------- | --------- | ---------------------
   sourceAddress|String|选填，操作源账户地址
   initBalance|int64|必填，给合约账户的初始化资产，单位UGas，1 Gas = 10^8 UGas, 大小限制[1, max(int64)]
...
- 成员
   ...
   sourceAddress|String|选填，操作源账户地址
   contractAddress|String|必填，合约账户地址
   gasAmount|int64|选填，资产发行数量，大小限制[0, max(int64)]，当0时仅触发合约
   input|String|选填，待触发的合约的main()入参
   metadata|String|选填，备注
...
- **响应数据**
   ...
   --------- | ------------- | ---------------- 
   address	  |    String     |    账户地址       
   balance	  |    int64       |    账户余额，单位UGas，1 Gas = 10^8 UGas, 必须大于0
...
- **响应数据**
   ...
   参数      |     类型     |        描述       
   ----------- | ------------ | ---------------- 
   balance     |   int64       | Gas的余额，单位UGas，1 Gas = 10^8 UGas, 
...
- **示例**
   ...
   var address string = "adxSmZHjDopQam2y5ntvHhGk4XEkiXc9MnFWZ"
   reqData.SetAddress(address)
...
- **示例**
   ...
   var address string = "adxSmZHjDopQam2y5ntvHhGk4XEkiXc9MnFWZ"
   reqData.SetAddress(address)
   reqData.SetIssuer("adxSdV6qWfyhJAmzrPbv3btsPPT6aFrqBku3C")
   reqData.SetCode("HNC")
...
- **示例**
   ...
   var address string = "adxSYQ8iMyZ7Dkj1oX1kjGMV55WXvoPKcLEK3"
   reqData.SetAddress(address)
...
- **请求参数**
   ...
   contractBalance|int64|选填，赋予合约的初始 Gas 余额, 单位UGas，1 Gas = 10^8 UGas, 大小限制[1, max(int64)]
...
priv|[Priv](#priv)|账户权限
metadata|`[]`[Metadata](#metadata)|账户
initBalance|int64|账户资产, 单位UGas，1 Gas = 10^8 UGas, 
...
| gasPrice    | int64 | 交易燃料单价，单位UGas，1 Gas = 10^8 UGas |
